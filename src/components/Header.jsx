...
const Header = () => {
  ...
  useEffect(()=>{
    if(selectedFile){
      uploadImageToStroage()
    }
  },[selectedFile])

  async function uploadImageToStroage(){
          ...
          <div className="flex flex-col items-center h-[100%]  justify-center">
            {
              selectedFile ? (
                ...
                src={imageFileUrl} alt="selectedfile" className={`w-full max-h-[250px] object-cover cursor-pointer ${imageFileUploading ? 'animate-pulse' : '' }`} />
              ) : (
